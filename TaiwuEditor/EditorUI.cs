...
namespace TaiwuEditor
{
    internal class EditorUIOld : MonoBehaviour
    {
        ...
        internal static bool Load(Settings instance)
        {
            if (instance == null)
            {
                TaiwuEditor.Logger.LogWarning("[TaiwuEditor] UI.Load() Settings instance is null");
                return false;
            }
            try
            {
                ...
                if (Instance == null)
                {
                    ...
                    {
                        TaiwuEditor.Logger.LogWarning("[TaiwuEditor]UI Background Texture Loading Failure");
                    }
                    return true;
                }
            }
            ...
            {
                TaiwuEditor.Logger.LogError("[TaiwuEditor]UI Loading Failure");
        ...
        {
            // 设置标题
            GUILayout.Label($"太吾修改器 {TaiwuEditor.version}", titleStyle, GUILayout.Width(mWindowWidth));
            GUILayout.Space(3f);
            SetFuncMenu();
            DrawList();
        ...
        public void DrawList()
        {
            if (showList == true)
            {
                ...
                {
                    TaiwuEditor.Logger.LogInfo($"rect: {rect.x} {rect.y} {rect.height} {rect.width}");
                    //为了留出最方下的横线,这里高度减1
