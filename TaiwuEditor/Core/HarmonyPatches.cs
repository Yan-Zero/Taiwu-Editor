...
namespace TaiwuEditor
{
    public static class HarmonyPatches
    {
        ...
        public static readonly Dictionary<string, PatchHandler> PatchHandles = new Dictionary<string, PatchHandler>
        {
            { "无限特质点", new PatchHandler
            {
                TargetType = typeof(UI_NewGame),
                TargetMethonName = "UpdatePoints",
                Postfix = AccessTools.Method(PatchesType,"NewGame_InfAbilityP")
            }},
            //{ "时钟", new PatchHandler
            //{
        ...
        /// </summary>
        /// <param name="__result"></param>
        private static void NewGame_InfAbilityP(ref int[] ____selectedPoints, ref UI_NewGame __instance)
        {
            if (TaiwuEditor.enabled && TaiwuEditor.settings.InfAbilityP.Value)
                ____selectedPoints[3] = 0;
        }
